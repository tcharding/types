...
// Get value at index
func (fen *Fenwick) Get(index int) int {
	if index == 0 {
		return fen.Sum(index)
	}
	return fen.Sum(index) - fen.Sum(index-1)
}

// Set index to value.
func (fen *Fenwick) Set(index, value int) {
	fen.Update(index, value-fen.Get(index))
}
